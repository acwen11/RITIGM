    ...
    for(int j=jmin;j<jmax;j++) {
      for(int i=imin;i<imax;i++) {
        ...
        if( rhoL < rho_min_atm * (1 + thresh_tol) || rhoL > rho_max_threshold ) {
          ...
          // Step 3.h: Compute u_{mu} = g_{mu nu}u^{nu}
          // Step 3.h.i: Set g_{mu nu}
          // Step 3.h.i.1: Set gamma_{ij}
          ...
          // Step 3.h.i.3: Compute beta^{2} = beta_{i}beta^{i}
          CCTK_REAL betasqr = 0.0;
          for(int ii=0;ii<3;ii++) betasqr += betaD[ii]*betaU[ii];

          // Step 3.h.i.4: Set g_{mu nu}
          CCTK_REAL g4DD[4][4];
          ...
          // Step 3.h.ii: Compute u_{mu} = g_{mu nu}u^{nu}
          ...
          for(int mu=0;mu<4;mu++) usqr += u4D[mu]*u4U[mu];
					...
					// Suppressing this warning. Deviations are roundoff level and allegedly unimportant.
