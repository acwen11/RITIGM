...
def read_and_slice_EOS_table_at_given_temperature(eos_file_path, T_in_MeV):
    ...
    for rho_idx in range(len(tab_rho)):
        ...
        # print("Setting slice idx %d of %d."%(rho_idx, len(tab_rho)))
        # print("Density = %f"%(tab_rho[rho_idx]))
        # print(munu[:,T_idx,rho_idx])
    ...
    # If table does not reach beta equilibrium at certain densities:
    if num_valid != len(Ye_be):
        tab_rho = tab_rho[:num_valid]
        Ye_be  = Ye_be[:num_valid]
        P_be   = P_be[:num_valid]
        eps_be = eps_be[:num_valid]
