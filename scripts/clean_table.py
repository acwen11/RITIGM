...
output_tablepath   = original_tablepath.split(".h5")[0]+"_cleaned.h5"
...
# Step 0c.i: Unified atomic mass unit (MeV)
UAMU_MEV      = 931.494061
...
UAMU_CGS      = UAMU_MEV * MEV_CGS / (C_CGS**2)
...
# Step 2c: Set eps: table entry is log10(eps + eps_{0})
...
# SKIP Step 3: Set the new mass factor

# Step 4: Compute the total energy density: e = rho(1+eps)
energy = rho*(1+eps/(C_CGS**2))

# SKIP Step 5: Adjust rho

# SKIP Step 6: Adjust eps
...
table["logenergy"   ][...] = np.log10(eps + eps0)
# Step 8c: Sound speed
table["cs2"         ][...] = cs2*h
# SKIP Step 8d: Energy shift
...
with h5.File(output_tablepath,"r") as table:
    ...
    rho   = 10**np.array(table["logrho"      ])[:]
    press = 10**np.array(table["logpress"    ])[:]
    eps   = 10**np.array(table["logenergy"   ])[:]
    cs2   =     np.array(table["cs2"         ])[:]
    eps0  =              table["energy_shift"][0]
