...
extern "C" void IllinoisGRMHD_driver_evaluate_MHD_rhs(CCTK_ARGUMENTS) {
  ...
  if(CCTK_Equals(verbose, "essential+iteration output")) {
    CCTK_VInfo(CCTK_THORNSTRING,"***** Iter. # %d, Lev: %d, Integrating to time: %e *****",cctk_iteration,levelnumber,cctk_delta_time/cctk_timefac+cctk_time);
  }
...
#pragma omp parallel for
  ...
  if( eos.is_Tabulated ) {
    ...
    compute_remaining_prims_on_right_and_left_face(eos,cctkGH,cctk_lsh,r,in_prims,out_prims_r,out_prims_l);
  }
