...
#pragma omp parallel for
  for(int k=cctk_nghostzones[2];k<cctk_lsh[2]-(cctk_nghostzones[2]-1);k++) for(int j=cctk_nghostzones[1];j<cctk_lsh[1]-(cctk_nghostzones[1]-1);j++) for(int i=cctk_nghostzones[0];i<cctk_lsh[0]-(cctk_nghostzones[0]-1);i++) {
	...
	if( eos.evolve_entropy ) {
		apply_floors_and_ceilings_to_prims__recompute_prims(eos,METRIC_LAP_PSI4,Ur,METRIC[SHIFTX],METRIC[SHIFTY],METRIC[SHIFTZ],rho_b_atm, T_atm);
		apply_floors_and_ceilings_to_prims__recompute_prims(eos,METRIC_LAP_PSI4,Ul,METRIC[SHIFTX],METRIC[SHIFTY],METRIC[SHIFTZ],rho_b_atm, T_atm);
	}
