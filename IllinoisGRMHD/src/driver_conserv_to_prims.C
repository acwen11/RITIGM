...
#pragma omp parallel for reduction(+:failures,vel_limited_ptcount,font_fixes,pointcount,failures_inhoriz,pointcount_inhoriz,error_int_numer,error_int_denom,rho_star_fix_applied,atm_resets,backup1,backup2,backup3,num_of_conservative_averagings_needed,nan_found) schedule(static)
				...
				for(int i=imin;i<imax;i++) {
					...
					if( (loop_count == 0) || (c2p_fail_flag != 0) ) {
						...
						if( check == 0 ) {
							...
							if (update_HydroBase) {
								CCTK_VINFO("Updating HydroBase after C2P.");
           			...
           			// Read from main memory
				        ...
				        // CCTK_REAL lapseL = alp[index];
								...
								// vel[CCTK_GFINDEX4D(cctkGH,i,j,k,0)] = (PRIMS[VX] + METRIC[SHIFTX])*lapseL_inv;
        				// vel[CCTK_GFINDEX4D(cctkGH,i,j,k,1)] = (PRIMS[VY] + METRIC[SHIFTY])*lapseL_inv;
        				// vel[CCTK_GFINDEX4D(cctkGH,i,j,k,2)] = (PRIMS[VZ] + METRIC[SHIFTZ])*lapseL_inv;
	...
	if( ( (conserv_to_prims_debug==1) && (error_int_numer/error_int_denom > 0.05) ) ||
		...
		myfile.write((char*)&rho_b_atm_max,                       1*sizeof(CCTK_REAL));
		myfile.write((char*)&tau_atm_hybrid,                      1*sizeof(CCTK_REAL));
