...
extern "C"
void Seed_Magnetic_Fields_Func (CCTK_ARGUMENTS)
{
...
#pragma omp parallel for
    for(int k=0;k<cctk_lsh[2];k++) for(int j=0;j<cctk_lsh[1];j++) for(int i=0;i<cctk_lsh[0];i++) {
	  ...
	  double r1_at_Ax_stagger = sqrt( x1*x1 + (yL+0.5*dY)*(yL+0.5*dY) + (zL+0.5*dZ)*(zL+0.5*dZ) );
	  double r1_at_Ay_stagger = sqrt( (x1+0.5*dX)*(x1+0.5*dX) + yL*yL + (zL+0.5*dZ)*(zL+0.5*dZ) );

	  double r2_at_Ax_stagger = sqrt( x2*x2 + (yL+0.5*dY)*(yL+0.5*dY) + (zL+0.5*dZ)*(zL+0.5*dZ) );
	  double r2_at_Ay_stagger = sqrt( (x2+0.5*dX)*(x2+0.5*dX) + yL*yL + (zL+0.5*dZ)*(zL+0.5*dZ) );
		...
		if(r1<=r_NS1 && r1>=r_NS1_low ) 
		  {
		    ...
		    Avec[CCTK_GFINDEX4D(cctkGH,i,j,k,0)] = -(yL + 0.5*dY)*A_b_x1*pow(max(Pressure_at_Ax_stagger-P_cut,0.0),n_s);
		  }
		if(r2<=r_NS2 && r2>=r_NS2_low ) 
	          {
		    Avec[CCTK_GFINDEX4D(cctkGH,i,j,k,1)] =  (x2 + 0.5*dX)*A_b_y2*pow(max(Pressure_at_Ay_stagger-P_cut,0.0),n_s);
		    Avec[CCTK_GFINDEX4D(cctkGH,i,j,k,0)] = -(yL + 0.5*dY)*A_b_x2*pow(max(Pressure_at_Ax_stagger-P_cut,0.0),n_s);
		  }
