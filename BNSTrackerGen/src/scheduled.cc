...
extern "C" void BNSTrackerGen_MoveSurfaces(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  else if (*bns_merger_stage == BNSTrackerGen::MERGING) {
    valid[2] = true;
    xc[2]    = *bns_cms_x;
    yc[2]    = *bns_cms_y;
    zc[2]    = move_z ? (*bns_cms_z) : 0.0;
  }
  else if (*bns_merger_stage == BNSTrackerGen::COLLAPSING) {
    valid[2] = true;
    xc[2]    = *bh_x;
    yc[2]    = *bh_y;
...
{  
  ...
  if (*bns_merger_stage == BNSTrackerGen::COLLAPSING) {
